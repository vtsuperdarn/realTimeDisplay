...
class DetailBox extends JFrame {
    ...
    private static final long serialVersionUID = 0x5253505047000015L;
    ...
    public DetailBox( String title, FanPlot plot ) {
    ...
    // determine the details at location i, j.
    // search the beams for the rangeGate containing this point
    // and display the data in the frame
    public void showDetails( int xi, int yi ) {
        
        Dimension d = plot.getSize();
        
        double x = 2.0 *  xi / d.width - 1.0;
        double y = 2.0 -  2.0 *  yi / d.height - 1.0;
        
        TreeMap beams = plot.getBeams();
        
        // draw the filled gates.
        Collection bc = beams.values();
        DetailData dd;
        
        //System.out.println( "showDetails "+x+" "+y);
        
        for ( Iterator i = bc.iterator(); i.hasNext(); ) {
            Beam beam = (Beam) i.next();
            
            if ( (dd = beam.findDetails( x, y )) != null ) {
